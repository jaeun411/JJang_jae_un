...
//-------------------------------------------------------------------------------------------------------------------------
//glb파일 로드/ 오브젝트 관리
const Model = ({ url,onObjectClick,setnewgltf, setText }) => {
...
const ThreeJs = ({ gltfBlobUrl, buildingId, floorNum, JsonData  }) => {
    ...
    const setnewgltf = (newgltf) => {
        ...
        const str = JsonData.split('ploygon')
        console.log(str);
    ...
    useEffect(()=>{
        ...
        // console.log(objectJson.ploygon0);
    ...
    const handleSubmitDetails = () => {
        if (selectedObject) {
            ...
            console.log("저장버튼 눌렀을 때 넘어오는 객체:",selectedObject);
            //오브젝트와, 넣을 이름을 넘겨준다
    ...
    return (
        <div style={{marginLeft: "150px"}}>
            <div>
                <Canvas
                    style={{height: "1000px", width: "1500px", marginLeft: "20px"}}
                >
                    <OrbitControls/>
                    <ambientLight intensity={100.0}/>
                    <pointLight position={[10, 10, 10]} intensity={1000}/>
                    {/* <Button onClick ={()=>setText(objects)}>새로고침</Button> */}
                    {gltfBlobUrl && <Model url={gltfBlobUrl} onObjectClick={handleObjectClick} setnewgltf={setnewgltf} setText={setText}/>}
                    {labels!=null&&Object.entries(labels).map(([uuid, label]) => (
                        <Text key={uuid}
